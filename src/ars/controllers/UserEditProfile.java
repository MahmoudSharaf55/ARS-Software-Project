...
public class UserEditProfile implements Initializable {
    ...
    public void onUserEditProfile() {
        ...
        //String emailUser =editUserMail.getText();
        ...
        if (! editUserName.getText().isEmpty() && editUserDOB.getValue() != null && editUserGender.getSelectionModel().getSelectedItem() != null && editUserMail.getText().contains("@") && !oldPassword.getText().isEmpty()&&!newPassword.getText().isEmpty()) {
            int flag = 0;
    ...
    public void changeSidePaneLabel(){
        try {
            VBox vBox;
            vBox = (VBox) FXMLLoader.load(getClass().getResource("../fxml/user_sidepane.fxml"));
                        ...
                        ((BorderPane) node).getChildren().forEach(node1 -> {
                            if (node1.getAccessibleText() != null) {
                                switch (node1.getAccessibleText()) {
                                    case "sideNameLbl":
                                        System.out.println(((Label)node1).getText());
                                        ((Label) node1).setText(AuthUser.currentUser.getName());
                                       System.out.println(((Label)node1).getText());
                                       System.out.println(AuthUser.currentUser.getName());
                                        break;
                                }
                            }
                        });
